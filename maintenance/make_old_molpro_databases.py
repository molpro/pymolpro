...
for subset in subsets:
    ...
    for master_file in master_files:
        ...
        for title in root.xpath('//db:database/@title', namespaces={'db': 'http://www.molpro.net/schema/molpro-database',
        ...
        for molecule_xml in root.xpath('//xi:include/@href',
            ...
            if cmlnode.get('formalCharge') and cmlnode.get('formalCharge') != '0': db.molecules[index]['charge'] = int(
                cmlnode.get('formalCharge'))
            if cmlnode.get('spinMultiplicity') and cmlnode.get('spinMultiplicity') != '1': db.molecules[index][
                'spin'] = int(cmlnode.get('spinMultiplicity')) - 1
        ...
        if name in ['benchmarks_N2minimal']: # special case of diatomic potential curves
            ...
            for reaction_node in root.xpath('//db:database/db:reaction',
                ...
                for species_node in reaction_node.xpath('db:species',
                    ...
                    if count: stoichiometry[species] = int(count)
                print('stoichiometry', stoichiometry,energy)
                db.add_reaction(reaction_node.get('index') if reaction_node.get('index') is not None else reaction_node.get('title'), stoichiometry,description=reaction_node.get('title'),energy=energy)
